...
class Forms {
    ...
    static async getCurrFormListUrl(baseIdxArchiveUrl) {
        let currQtr = Math.ceil(new Date().getMonth() / 4);
        let currYear = new Date().getFullYear();
        let formListUrl = `${baseIdxArchiveUrl}/${currYear}/QTR${currQtr}/xbrl.idx`
        return formListUrl;
    }
    ...
    static async getFormLists(formListUrls, baseXbrlListSavePath) {
        async function checkNextDir(formListUrls) {
            ...
            fs.writeFileSync(`${baseXbrlListSavePath}/${formUrl.split('/')[6] + '-' + formUrl.split('/')[7] + '-' + formUrl.split('/')[8]}`, formData);
    ...
    static async loadFormList(formListFileName, baseXbrlListSavePath) {
        let companyForms = fs.readFileSync(`${baseXbrlListSavePath}/${formListFileName}`, 'utf8').toString().split(/\n/).map(e => ({
            ...
            'formType': e.split('|')[2],
            'date': e.split('|')[3],
            'formPath': e.split('|')[4] ? e.split('|')[4].split(/[/|\-|.]/).reduce((r, e, i) => (i === 2 || i === 5) ? r + e + '/' : (i === 3 || i === 4) ? r + e : r + '', '/') : 0
        }));
        return companyForms;
    }
